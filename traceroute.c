/*
    PathDiv version 1.0 is a tool that can trace Multi-homed, multipath networks
    and calculates the Path Diversity in the context of rMPTCP protocol
    developed at the university of Duisburg-Essen. See Readme_PathDiv for details.

    Copyright (c) 2017                  Imranullah Khan
					(imranullah.khan@stud.uni-due.de)
   
    The tool is based on traceroute-2.1.0 developed by Dmitry Butskoy
...
//My temporary char array and index for it
char *tmpr[30] = { NULL };
int ind;
int topIndex = -1;
char *Pinf11[30] = { NULL };
...
//My: Path Diversity formula for two subflows
float PD_formula( char * PI1[], int PI1_size, char * PI2[], int PI2_size)
{
	int k=0;
	int n1, n2;
	for( n1=0; n1<PI1_size; n1++)
	{
		if(PI1[n1]==NULL)
			break;
	}

	for( n2=0; n1<PI2_size; n2++)
	{
		if(PI2[n2]==NULL)
			break;
	}
	...
	printf("Number of common nodes = k = %d", k);
...
static int S_if, D_if, i, j;
static void do_it (void);
static void PathInformation (void);
static void PathDiversity (void);
...
int main (int argc, char *argv[]) {
	...
	//get nr of source and destination interfaces from the terminal (argc and argv)		
...
//	int S_if , D_if, i,j;
	...
	printf ("\n***The PathDiv Tool***\n\nCurrent multi-homed configuration is:\n");
		
	printf ("No of Source interfaces:      %d\n", S_if);
	printf ("No of Destination interfaces: %d\n", D_if);
        printf ("Its a %d x %d configuration\n", S_if, D_if);
	printf ("------------------------------------------------------------------------------\n");
	ips s_ips[S_if], d_ips[D_if];
		
	printf("Please enter IP addresses and domain names only\n");
	printf("Sequence of entry sets the indices to identify subflows and related measurements\n");
		
	//Enter all source IP addresses
	for (i=0, j=1; i<S_if; i++, j++)
	{
	printf("Enter source addres number %d: ", j);
	scanf ("%s", s_ips[i].ip);
	}
	printf("\n");

	//Enter all destination IP addresses
	for (i=0, j=1; i<D_if; i++, j++)
	{
	printf("Enter destination address number %d: ", j);
	scanf ("%s", d_ips[i].ip);
	}

	//argv[1]="-T";  //for tcp-traceroute and change remaining argments accordingly
	// According to the command "traceroute -T -s "source-address" "destination-address" 
	...
	for (i=0; i<D_if ; i++)
	{
		for(j=0;j<S_if;j++)
		{
			argv[2]= s_ips[j].ip;
			argv[3]= d_ips[i].ip; 
			ind=0;  //My tmp index initiallized
		
			printf ("\n-------------------------------------------------------------------------------\n");
			int i2=i+1;
			int j2=j+1;
			printf ("Path information for Subflow%d%d-> from %s to %s \n", j2, i2, argv[2], argv[3]);
		...
		do_it ();  // Send, receiv & check probes: store result in *res (struct addrinfo): display ttl/hops, address of hops and RTT

		PathInformation(); //Display Path Information (trace) , save results and display it (Pinfs)



			} //Source Control loop ends here
		} // Destination Control loop ends here


printf("\n-----------------------------------------------------------------------------\n");


PathDiversity (); // PathDivesity function
...
static void pushInStack(char *str) {
	topIndex = topIndex + 1;
	int totalLength = 0;
	totalLength += strlen(str);
	tmpr[topIndex] = (char*) malloc(totalLength);
	strcpy(tmpr[topIndex], str);
	//printf ("\n %d -> %s  ->",topIndex, tmpr[topIndex]);
}
...
static void print_addr (sockaddr_any *res) {
	...
	if (noresolve)
	    ...
	    //printf ("\n%d-> %s (%s)",ind, buf[0] ? buf : str, str);
	    //*if (tmpr[ind]==NULL)
		
		//printf("tmpr %s=", tmpr[topIndex]);	
	       // tmpr[ind]=str;
		//}*/
	    
	//printf ("\n %d -> %s  ->",topIndex, tmpr[topIndex]); //My
...
/*printf("\n Print_ad array is %s, %s, %s, %s, %s, %s ",tmpr[0],tmpr[1],tmpr[2],tmpr[3],tmpr[4],tmpr[5]);*/
...
static void print_probe (probe *pb) {
	...
	if (!pb->res.sa.sa_family)
		{int donothing;
		//printf("\n While--The final array is %s, %s, %s, %s, %s, %s ",tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5]);
}
	else {
...
static void do_it (void) {
	...
	while (start < end) {
		...
		if (pb->done) {		
			...
			print_probe (pb);  // All printing donw here.
			start++;
		    }
		...
		//printf("\n For--The final array is %s, %s, %s, %s, %s, %s ",tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5]);		
	//printf("\nFor ind %d -> %s ",ind, tmp[ind]); //My for check
	...
	//printf("\n index = %d->", ind);
	//printf("\n End of do It is %s, %s, %s, %s, %s, %s ",tmpr[0],tmpr[1],tmpr[2],tmpr[3],tmpr[4],tmpr[5]);

	/*for (int in=0; in<=topIndex; in++)
	{
	printf("\n %d -> %s",in, tmpr[in]);
	}*/
...
// Display Path Information (trace), save it in Pinfs arrays
...
static void PathInformation (void)
{
  ...
  if (j==0 && i==0) // Pinf11
{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
	{
	int totalLength = 0;
	totalLength += strlen(tmpr[ind]);
	Pinf11[ind] = (char*) malloc(totalLength);
	strcpy(Pinf11[ind],tmpr[ind]);
	printf("\nPinf11[%d]= %s",ind, Pinf11[ind]);
	}
}
  else if(j==0 && i==1) // Pinff12
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
	{
		//printf("0x1");
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf12[ind] = (char*) malloc(totalLength);
		strcpy(Pinf12[ind],tmpr[ind]);
		printf("\nPinf12[%d]= %s",ind, Pinf12[ind]);
	}
}
  else if(j==0 && i==2) // Pinff13
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
	{
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf13[ind] = (char*) malloc(totalLength);
		strcpy(Pinf13[ind],tmpr[ind]);
		printf("\nPinf13[%d]= %s",ind, Pinf13[ind]);
		}

	}
  else if(j==1 && i==0) // Pinff21
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
		{
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf21[ind] = (char*) malloc(totalLength);
		strcpy(Pinf21[ind],tmpr[ind]);
		printf("\nPinf21[%d]= %s",ind, Pinf21[ind]);
		}

	}
  else if(j==1 && i==1) // Pinff22
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
		{
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf22[ind] = (char*) malloc(totalLength);
		strcpy(Pinf22[ind],tmpr[ind]);
		printf("\nPinf22[%d]= %s",ind, Pinf22[ind]);
		}

	}
  else if(j==1 && i==2) // Pinff23
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
		{
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf23[ind] = (char*) malloc(totalLength);
		strcpy(Pinf23[ind],tmpr[ind]);
		printf("\nPinf23[%d]= %s",ind, Pinf23[ind]);
		}

	}
  else if(j==2 && i==0) // Pinff31
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
		{
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf31[ind] = (char*) malloc(totalLength);
		strcpy(Pinf31[ind],tmpr[ind]);
		printf("\nPinf31[%d]= %s",ind, Pinf31[ind]);
		}

	}
  else if(j==2 && i==1) // Pinff32
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
		{
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf32[ind] = (char*) malloc(totalLength);
		strcpy(Pinf32[ind],tmpr[ind]);
		printf("\nPinf32[%d]= %s",ind, Pinf32[ind]);
		}

	}
  else if(j==2 && i==2) // Pinff33
	{
	for (ind= 0; ind<=topIndex; ind = ind + 1)
		{
		int totalLength = 0;
		totalLength += strlen(tmpr[ind]);
		Pinf33[ind] = (char*) malloc(totalLength);
		strcpy(Pinf33[ind],tmpr[ind]);
		printf("\nPinf33[%d]= %s",ind, Pinf33[ind]);
		}
...
static void PathDiversity (void)
{
//For 3x3 case, we get 9 Pinf for 9 subflows. Apply PD formula-> each vs rest
//For 2x2 case, we get 4 Pinfs

//Pinf3x3-> 11 , 12 , 13 , 21, 22, 23 , 31  ,32 , 33
//Pinf2x2-> 11 , 12 ,    , 21, 22,    ,     ,   , 

/* [D11-12], D11-13, [D11-21], [D11-22], D11-23, D11-31, D11-32, D11-33 ... D12-13, [D12-21], [D12-22], D12-23, D12-31, D12-32, D12-33 ... D13-21, D13-22, D13-23, D13-31, D13-32, D13-33 ... [D21-22], D21-23, D21-31, D21-32, D21-33 ... D22-23, D22-31, D22-32, D22-33 ... D23-31, D23-32, D23-33 ... D31-32, D31-33 ... D32-33
D11-12, D11-21, D11-22 ... D12-21, D12-22 ... D21-22 */

float D11_12, D11_21, D11_22, D12_21, D12_22, D21_22;
float PD11, PD12, PD13, PD21, PD22, PD23, PD31, PD32, PD33;

float D11_13, D11_23, D11_31, D11_32,D11_33, D12_13, D12_23, D12_31, D12_32, D12_33, D13_21, D13_22, D13_23, D13_31, D13_32, D13_33, D21_23, D21_31, D21_32, D21_33, D22_23, D22_31, D22_32, D22_33, D23_31, D23_32, D23_33, D31_32, D31_33, D32_33;

if (S_if>1 && D_if>1)   // Either 2x2 or 3x3
{
	
	D11_12= PD_formula(Pinf11, 30, Pinf12, 30);
	D11_21= PD_formula(Pinf11, 30, Pinf21, 30);
	D11_22= PD_formula(Pinf11, 30, Pinf22, 30);
	D12_21= PD_formula(Pinf12, 30, Pinf21, 30);
	D12_22= PD_formula(Pinf12, 30, Pinf22, 30);
	D21_22= PD_formula(Pinf21, 30, Pinf22, 30);
	
	if(S_if>2 && D_if>2) // its a 3x3 case
	{
		 
		D11_13= PD_formula(Pinf11, 30, Pinf13, 30);
		D11_23= PD_formula(Pinf11, 30, Pinf23, 30);
		D11_31= PD_formula(Pinf11, 30, Pinf31, 30);
		D11_32= PD_formula(Pinf11, 30, Pinf32, 30);
		D11_33= PD_formula(Pinf11, 30, Pinf33, 30);
		D12_13= PD_formula(Pinf12, 30, Pinf13, 30);
		D12_23= PD_formula(Pinf12, 30, Pinf23, 30);
		D12_31= PD_formula(Pinf12, 30, Pinf31, 30);
		D12_32= PD_formula(Pinf12, 30, Pinf32, 30);
		D12_33= PD_formula(Pinf12, 30, Pinf33, 30);
		D13_21= PD_formula(Pinf13, 30, Pinf21, 30);
		D13_22= PD_formula(Pinf13, 30, Pinf22, 30);
		D13_23= PD_formula(Pinf13, 30, Pinf23, 30);
		D13_31= PD_formula(Pinf13, 30, Pinf31, 30);
		D13_32= PD_formula(Pinf13, 30, Pinf32, 30);
		D13_33= PD_formula(Pinf13, 30, Pinf33, 30);
		D21_23= PD_formula(Pinf21, 30, Pinf23, 30);
		D21_31= PD_formula(Pinf21, 30, Pinf31, 30);
		D21_32= PD_formula(Pinf21, 30, Pinf32, 30);
		D21_33= PD_formula(Pinf21, 30, Pinf33, 30);
		D22_23= PD_formula(Pinf22, 30, Pinf23, 30);
		D22_31= PD_formula(Pinf22, 30, Pinf31, 30);
		D22_32= PD_formula(Pinf22, 30, Pinf32, 30);
		D22_33= PD_formula(Pinf22, 30, Pinf33, 30);
		D23_31= PD_formula(Pinf23, 30, Pinf31, 30);
		D23_32= PD_formula(Pinf23, 30, Pinf32, 30);
		D23_33= PD_formula(Pinf23, 30, Pinf33, 30);
		D31_32= PD_formula(Pinf31, 30, Pinf32, 30);
		D31_33= PD_formula(Pinf31, 30, Pinf33, 30);
		D32_33= PD_formula(Pinf32, 30, Pinf33, 30);
	}
}

printf ("\n---------------------------------------------------------------------------");
if (S_if==2 && D_if==2)   // Either 2x2 or 3x3
	{
	printf("\nA dump for two-paths path diversity values\n"); 
	printf("D11_12= %f\nD11_21= %f\nD11_22= %f\nD12_21= %f\nD12_22= %f\nD21_22= %f", D11_12, D11_21, D11_22, D12_21, D12_22, D21_22);

printf("\n----------------------------------------------------------------------------\n");

	printf ("\n PathDiversity metric for Subflow11= PD11 = %f\n", D11_22);
	printf ("\n PathDiversity metric for Subflow22= PD22 = %f\n", D11_22);
	printf ("\n PathDiversity metric for Subflow12= PD12 = %f\n", D12_21);
	printf ("\n PathDiversity metric for Subflow21= PD12 = %f\n", D12_21);
	printf ("\n\n Note: PD11=PD22 and PD12=PD21\n");

}
else if(S_if>2 && D_if>2) // its a 3x3 case
{
		printf("\nA dump of two-paths path diversity values\n");
	printf("\nD11_12= %f, D11_13= %f, D11_21= %f, D11_22= %f, D11_23= %f, D11_31= %f, D11_32= %f, D11_33= %f, D12_13= %f, D12_21= %f, D12_22= %f, D12_23= %f, D12_31= %f, D12_32= %f, D12_33= %f, D13_21= %f, D13_22= %f, D13_23= %f, D13_31= %f, D13_32= %f, D13_33= %f, D21_22= %f, D21_23= %f, D21_31= %f, D21_32= %f, D21_33= %f, D22_23= %f, D22_31= %f, D22_32= %f, D22_33= %f, D23_31= %f, D23_32= %f, D23_33= %f, D31_32= %f, D31_33= %f, D32_33=%f\n", D11_12, D11_13, D11_21, D11_22, D11_23, D11_31, D11_32, D11_33, D12_13, D12_21, D12_22, D12_23, D12_31, D12_32, D12_33, D13_21, D13_22, D13_23, D13_31, D13_32, D13_33, D21_22, D21_23, D21_31, D21_32, D21_33, D22_23, D22_31, D22_32, D22_33, D23_31, D23_32, D23_33, D31_32, D31_33, D32_33);
}

// PathDiversity calculation for Multipath 2x2 or 3x3 cases

printf("----------------------------------------------------------------------------\n");

if(S_if>2 && D_if>2)
{
		PD11= (D11_22+D11_23+D11_32+D11_33)/4;
		PD12= (D12_21+D12_23+D12_31+D12_33)/4;
		PD13= (D13_21+D13_22+D13_31+D13_32)/4;
		PD21= (D12_21+D13_21+D21_32+D21_33)/4;
		PD22= (D11_22+D13_22+D22_31+D22_33)/4;
		PD23= (D11_23+D12_23+D23_31+D23_32)/4;
		PD31= (D12_31+D13_31+D22_31+D23_31)/4;
		PD32= (D11_32+D13_32+D21_32+D23_32)/4;
		PD33= (D11_33+D12_33+D21_33+D22_33)/4;

	//Print the PathDiversity values for 3x3 case

	printf("\n PathDiversity metric for Subflow11= PD11 = %f", PD11);
	printf("\n PathDiversity metric for Subflow12= PD12 = %f", PD12);
	printf("\n PathDiversity metric for Subflow13= PD13 = %f", PD13);
	printf("\n PathDiversity metric for Subflow21= PD21 = %f", PD21);
	printf("\n PathDiversity metric for Subflow22= PD22 = %f", PD22);	
	printf("\n PathDiversity metric for Subflow23= PD23 = %f", PD23);
	printf("\n PathDiversity metric for Subflow31= PD31 = %f", PD31);
	printf("\n PathDiversity metric for Subflow32= PD32 = %f", PD32);
	printf("\n PathDiversity metric for Subflow33= PD33 = %f\n", PD33);
}
...
static void FreeMemory (void)
{
//free the char pointers
int freeInd;

if (tmpr[0])
{
for (freeInd =0; tmpr[freeInd]; freeInd++)
	free(tmpr[freeInd]);
//free(tmpr[]);
}
...
if(Pinf11[0])
{
	for (freeInd =0; Pinf11[freeInd]; freeInd++)
		free(Pinf11[freeInd]);
	//free(Pinf11[]);
}
...
if(Pinf12[0])
{
for (freeInd =0; Pinf12[freeInd]; freeInd++)
	free(Pinf12[freeInd]);
//free(Pinf12);
}
...
if(Pinf13[0])
{
for (freeInd =0; Pinf13[freeInd]; freeInd++)
	free(Pinf13[freeInd]);
//free(Pinf13);
}
...
if(Pinf21[0])
{
for (freeInd =0; Pinf21[freeInd]; freeInd++)
	free(Pinf21[freeInd]);
//free(Pinf21);
}
...
if(Pinf22[0])
{
for (freeInd =0; Pinf22[freeInd]; freeInd++)
	free(Pinf22[freeInd]);
//free(Pinf22);
}
...
if(Pinf23[0])
{
for (freeInd =0; Pinf23[freeInd]; freeInd++)
	free(Pinf23[freeInd]);
//free(Pinf23);
}
...
if(Pinf31[0])
{
for (freeInd =0; Pinf31[freeInd]; freeInd++)
	free(Pinf31[freeInd]);
//free(Pinf31);
}
...
if(Pinf32[0])
{
for (freeInd =0; Pinf32[freeInd]; freeInd++)
	free(Pinf32[freeInd]);
//free(Pinf32);
}
...
if(Pinf33[0])
{
for (freeInd =0; Pinf33[freeInd]; freeInd++)
	free(Pinf33[freeInd]);
//free(Pinf33);
} 
} 
